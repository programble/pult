    ...
    fs.fstat(fd, function(err, stats) {
      ...
      if (req.headers['if-modified-since']) {
        ...
        if (stats.mtime <= since) {
          // FIXME: Need to register a no-op data handler so that the file
          // descriptor gets closed. Closing the file descriptor using
          // `fs.close` causes `EBADF` errors to be emitted on `readStream`.
